...
docker run -d --name otel-collector --network host \
  -v $(pwd)/otel-collector-config/config.yaml:/etc/otelcol-contrib/config.yaml \
...
# 参考 mr-collector.conf.example 或 conf/spark35-hadoop3-hive2.3.9/mr-collector.conf
```
...
将 `grafana/` 目录下的 JSON 文件导入 Grafana：
...
| Hive 面板 `Unknown column 'mr'` | Grafana 面板旧版本 | 重新导入 `grafana/hive-mr.json` |

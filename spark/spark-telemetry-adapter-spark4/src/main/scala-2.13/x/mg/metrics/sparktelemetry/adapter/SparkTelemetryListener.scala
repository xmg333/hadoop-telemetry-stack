...
/**
 * Spark 4.x Listener adapter.
 * Compiled against Spark 4.x + Scala 2.13.
 *
 * For Spark 4.x: uses ShuffleWriteMetrics.bytesWritten (same as 3.x API)
 */
class SparkTelemetryListener(confMap: Map[String, String]) extends SparkListener {
  ...
  override def onTaskEnd(taskEnd: SparkListenerTaskEnd): Unit = {
    ...
    // Shuffle write metrics (Spark 4.x API: bytesWritten, writeTime)

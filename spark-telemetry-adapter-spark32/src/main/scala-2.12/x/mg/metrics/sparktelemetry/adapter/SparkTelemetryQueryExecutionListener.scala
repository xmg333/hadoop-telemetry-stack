...
class SparkTelemetryQueryExecutionListener(confMap: Map[String, String]) extends QueryExecutionListener
   ...
   *
   * Spark 3.x AQE: AdaptiveSparkPlanExec and QueryStageExec extend LeafExecNode (children=Nil),
  ...
  private def visit(plan: SparkPlan, qm: SqlExecutionMetrics, tm: java.util.List[SqlTableIOMetrics]): Unit = {
    ...
    plan match {
      case a: AdaptiveSparkPlanExec =>
        ...
        // Use public inputPlan() method instead of reflection
        visit(a.inputPlan, qm, tm)
        return
      case q: QueryStageExec =>
        // QueryStageExec extends LeafExecNode, children=Nil
        // Use public plan() method instead of reflection
        visit(q.plan, qm, tm)
        return
      case _ =>
    }
  ...
  private def extractBatchScanTableName(scan: BatchScanExec): String = {
    ...
    try {
      val tableMethod = scan.getClass.getMethod("table")
      val table = tableMethod.invoke(scan)
      if (table != null) {
        val nameMethod = table.getClass.getMethod("name")
        val name = nameMethod.invoke(table)
